                            ...
                            <pagination
                                ...
                                <template v-if="!layerid" slot="searchBtnBefore">
                                    ...
                                    <div class="search-condition-row"> -->
                                         <span class="title title-text">所在分行：</span>
                                        <select-office-input
                                            placeholder="请点击选择"
                                            type="simple"
                                            :params="params"
                                            title="所在分行"
                                            v-model="otherParam.officeId"></select-office-input>
                                        <span class="title title-text">所在部门：</span>
                                        <layui-select
                                            style="display: inline-block;width: 138px;margin-right: 10px;"
                                            v-model="otherParam.departmentId"
                                            label="orgName"
                                            valKey="orgId"
                                            :items="departments"></layui-select>
                                        ...
                                        <span v-if="false" class="title title-text">所属中心：</span>
                                        <input v-if="false && !centerConfigOrg" type="text" readonly=true :value="personnelCenterConfigs[0].centerName" placeholder="所属中心">
                                        <layui-select
                                            v-if="false && centerConfigOrg"
                                            style="display: inline-block;width: 138px;margin-right: 10px;"
                                        ...
                                        <span class="title title-text">行员级别：</span>
                                        <!-- <input type="number" v-model="otherParam.currentLevelDict" placeholder="请输入行员级别"> -->
                                        <layui-select
                                            style="display: inline-block;width: 138px;"
                                            v-model="otherParam.currentLevelDict"
                                            :items="levels"></layui-select>
                                        <!-- <span class="title title-text">岗位：</span>
                                        <layui-select
                                ...
                                <template v-if="!layerid" slot="opera">
                                    <button v-authorize="'ROLE_ITPOM.PERSONNEL.BRANCH.ADMIN.ROLE'" @click="create" class="layui-btn layui-btn-icon layui-btn-primary"><i class="fa fa-plus" aria-hidden="true"></i>&nbsp;&nbsp;新增</button>
                                    <button v-if="false" v-authorize="'ROLE_PERSONNEL.ADMIN.USER.ROLE'" @click="importPersonnel" class="layui-btn layui-btn-icon layui-btn-primary"><i class="fa fa-cloud-upload" aria-hidden="true"></i>&nbsp;&nbsp;导入</button>
                                    <button v-authorize="'ROLE_ITPOM.PERSONNEL.BRANCH.ADMIN.ROLE'" @click="importPersonnel('list')" class="layui-btn layui-btn-icon layui-btn-primary"><i class="fa fa-cloud-upload" aria-hidden="true"></i>&nbsp;&nbsp;批量导入</button>
                                    <button v-if="false" v-authorize="'ROLE_PERSONNEL.ADMIN.USER.ROLE'" @click="exportPersonnel(false, 1)" class="layui-btn layui-btn-icon layui-btn-primary"><i class="iconfont icon-download" aria-hidden="true"></i>&nbsp;&nbsp;导出员工基本信息统计表</button>
                                    <button v-if="false" v-authorize="'ROLE_PERSONNEL.ADMIN.USER.ROLE'" @click="exportPersonnel(true, 1)" class="layui-btn layui-btn-icon layui-btn-primary"><i class="iconfont icon-download" aria-hidden="true"></i>&nbsp;&nbsp;导出部门全体员工信息汇总表</button>
                                    <button v-authorize="'ROLE_ITPOM.PERSONNEL.BRANCH.ADMIN.ROLE'" @click="adjustmentLevel()" class="layui-btn layui-btn-icon layui-btn-primary"><i class="fa fa-level-up" aria-hidden="true"></i>&nbsp;&nbsp;行员级别调整</button>
                                    <button v-if="false" v-authorize="'ROLE_PERSONNEL.ADMIN.USER.ROLE'" @click="adjustmentJob()" class="layui-btn layui-btn-icon layui-btn-primary"><i class="fa fa-briefcase" aria-hidden="true"></i>&nbsp;&nbsp;岗位调整</button>
                                    <button v-authorize="'ROLE_ITPOM.PERSONNEL.BRANCH.ADMIN.ROLE'" @click="importAssessment()" class="layui-btn layui-btn-icon layui-btn-primary"><i class="fa fa-briefcase" aria-hidden="true"></i>&nbsp;&nbsp;考核导入</button>
                                    <button v-if="false" v-authorize="'ROLE_PERSONNEL.ADMIN.USER.ROLE'" @click="importHxWork()" class="layui-btn layui-btn-icon layui-btn-primary"><i class="fa fa-briefcase" aria-hidden="true"></i>&nbsp;&nbsp;行内档案履历导入</button>
                                    <file-download-modal-button v-if="false" v-authorize="'ROLE_PERSONNEL.ADMIN.USER.ROLE'" text="模板下载" :data="templateFileData"></file-download-modal-button>
                                </template>
                                <template slot="main">
                                    <table class="footable table table-stripped">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style="text-align:center;width: 130px;">所在分行<field-sort @orderBefore="orderBefore" orderFieldName="customOrderField" field="OfficeLevel"></field-sort></th>
                                                <th style="text-align:center;width: 130px;">所在部门<field-sort @orderBefore="orderBefore" orderFieldName="customOrderField" field="Office"></field-sort></th>
												...
												<th style="text-align:center;width: 90px;">入行年限<field-sort @orderBefore="orderBefore" field="EntryHxTime"></field-sort></th>
                                        ...
                                        <tbody id="meetingTbody" class="footable_table">
                                            <template v-for="(item, index) in page.list">
                                                <tr 
													...
													<!-- <td style="text-align:center;">{{ officeNameMap[item.departmentId] | pv('orgName') }}</td> -->
                                                    <td style="text-align:center;" :title="officeNameMap[item.officeId] ? officeNameMap[item.officeId].orgDescription : ''">{{ officeNameMap[item.officeId] ? officeNameMap[item.officeId].orgDescription.split('/')[1] : '' }}</td>
                                                    <td style="text-align:center;" :title="officeNameMap[item.officeId] ? officeNameMap[item.officeId].orgDescription : ''">{{ officeNameMap[item.officeId] | pv('orgName') }}</td>
                                                    <td style="text-align:center;" :title="getJobInfo(item)">{{ getJobInfo(item) }}</td>
                                                    <td style="text-align:center;"><span title="点击查看历次行员级别变动历史记录" class="l-cursor" @click="showHiLevel(item, true)" style="color: rgb(30, 159, 255);">{{ levelMap[item.currentLevelDict] }}</span></td>
...
import { del, exportPersonInfo, getOfficesByCenterCode, getDepartments } from '@/api/personnel/personnel'
...
export default {
    ...
    watch: {
        ...
        'otherParam.officeId': function() {
            this.departments =  [];
            this.$set(this.otherParam, 'departmentId', null);
            let orgParentId = this.otherParam.officeId;
            if (!orgParentId) {
                return;
            }
            officeApi.gets({
                ...
                let data = res.data || {};
                this.departments = data.authOrgs || [];
            });
        },
        'otherParam.departmentId': function () {
            this.$set(this.otherParam, 'job', null);
            var officeId = this.otherParam.departmentId;
            this.jobs = [];
            if (officeId) {
                let officeName = this.departments[this.departments.indexOf(officeId, 'orgId')].orgName;
                getPersonnelJobManages({
                    officeName: officeName == "信息技术部" ? "信息科技部" : (officeName == "综合管理部" ? '运营管理部' : officeName),
                    jobManageType: 'fh',
                }).then(res => {
                    var data = res.data;
    ...
    methods: {
        getJobInfo(personnel) {
            let personnelJobInfos = this.jobInfoMap[personnel.id] || [];
            let jobInfo = "";
            personnelJobInfos.forEach((item, index) => {
                jobInfo += this.jobNameMap[item.jobCode] || '';
                if (index < (personnelJobInfos.length - 1)) {
                    jobInfo += '、';
                }
            });
            return jobInfo;
        ...
        initData: function() {
            getDicts('GENDER_DICT,PERSONNEL_EDUCATION_DICT,PERSONNEL_LEVEL_DICT').then(result => {
        ...
        handleSearchResult: function(data){
            ...
            this.jobInfoMap = data.jobInfoMap || {};
        },
        create: function() {
