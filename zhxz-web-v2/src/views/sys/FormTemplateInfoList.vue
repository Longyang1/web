                                    ...
                                    <table class="footable table table-stripped">
                                        <thead>
                                            <tr>
												...
												<th style="text-align:center;">模板使用年度</th>
												<th style="text-align:center;">模板业务类型</th>
												<th style="text-align:center;">模板发布状态</th>
												<th style="text-align:center;">模板发布时间</th>
												<th style="text-align:center;">模板复制来源ID</th>
                                        ...
                                        <tbody id="meetingTbody" class="footable_table">
                                            <template v-for="(item, index) in page.list">
                                                <tr 
													...
													<td style="text-align:center;">{{ item.templateYear }}</td>
													<td style="text-align:center;">{{ item.templateBizType }}</td>
													<td style="text-align:center;">{{ item.publishStatus }}</td>
													<td style="text-align:center;">{{ item.publishDate }}</td>
													<td style="text-align:center;">{{ item.copyTemplateId }}</td>
...
import { del } from '@/api/sys/formTemplateInfo'
...
export default {
    ...
    methods: {
        initData: function() {
            getDicts('FORM_CONFIG_TYPE_DICT,').then(result => {
            	//
        ...
        handleSearchResult: function(data){
            this.page = data.page || {};
            this.page['list'] = data.formTemplateInfos || [];
        },
        create: function() {
         	this.$parent.closeTabByName('FormTemplateInfoForm');
        ...
        edit: function(formTemplateInfo) {
         	this.$parent.closeTabByName('FormTemplateInfoForm');
            this.$parent.clickMenu({
                "checked": false,
                "icon": "fa-legal",
                ...
                id: formTemplateInfo.id,
            });
        },
        del: function(formTemplateInfo) {
            confirm('确定删除该记录吗？', () => {
                del(formTemplateInfo.id).then(result => {
                    alert("删除成功");
