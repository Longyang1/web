                                    ...
                                    <table class="footable table table-stripped">
                                        <thead>
                                            <tr>
												...
												<th style="text-align:center;">表单模板ID</th>
												<th style="text-align:center;">组件ID</th>
												<th style="text-align:center;">选项组件设置值</th>
												<th style="text-align:center;">结构组件设置值</th>
												<th style="text-align:center;">复合组件设置值</th>
                                        ...
                                        <tbody id="meetingTbody" class="footable_table">
                                            <template v-for="(item, index) in page.list">
                                                <tr 
													...
													<td style="text-align:center;">{{ item.templateId }}</td>
													<td style="text-align:center;">{{ item.componentId }}</td>
													<td style="text-align:center;">{{ item.optionComponentValue }}</td>
													<td style="text-align:center;">{{ item.tableComponentValue }}</td>
													<td style="text-align:center;">{{ item.multiComponentValue }}</td>
...
import { del } from '@/api/sys/formTemplateComponent'
...
export default {
    ...
    data () {
        return {
        	menuId: 'FormTemplateComponentList',
            titles: [],
            dicts: [],
            paramNames: [],
            otherParam: {
    ...
    methods: {
        ...
        handleSearchResult: function(data){
            this.page = data.page || {};
            this.page['list'] = data.formTemplateComponents || [];
        },
        create: function() {
         	this.$parent.closeTabByName('FormTemplateComponentForm');
            this.$parent.clickMenu({
                "checked": false,
                "icon": "fa-legal",
                "id": "formTemplateComponent",
                "name": "FormTemplateComponentForm",
                "parentId": "4",
                "text": "XX创建",
                "children": [
        ...
        edit: function(formTemplateComponent) {
         	this.$parent.closeTabByName('FormTemplateComponentForm');
            this.$parent.clickMenu({
                "checked": false,
                "icon": "fa-legal",
                ...
                id: formTemplateComponent.id,
            });
        },
        del: function(formTemplateComponent) {
            confirm('确定删除该记录吗？', () => {
                del(formTemplateComponent.id).then(result => {
                    alert("删除成功");
