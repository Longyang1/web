                    ...
                    <div class="float-e-margins">
                        <div id="center">
                            <pagination
                                ...
                                <template slot="opera">
                                    <button @click="create" class="layui-btn layui-btn-icon layui-btn-primary"><i class="fa fa-plus" aria-hidden="true"></i>&nbsp;&nbsp;新增</button>
                                </template>
                                <template slot="main">
...
export default {
    ...
    data () {
        return {
            ...
            otherParam: {
                companyBusinessTypeDict: 'customer',
            },
            refresh: true,
            page: {
    ...
    methods: {
        initData: function() {
            getDicts('REGISTE_RTYPE_DICT,SC_COUNTRY_DICT,CURRENCY_DICT,LINK_CARD_TYPE_DICT,SC_CARD_TYPE_DICT,COMPANY_SERVICE_STATE_DICT,').then(result => {
        ...
        del: function(bmCompanyInfo) {
            confirm('确定删除该记录吗？', () => {
                del(bmCompanyInfo.id).then(result => {
                    alert("删除成功");
